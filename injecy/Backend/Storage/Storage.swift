...
final class Storage: ObservableObject {
	...
	func clearContext<T: NSManagedObject>(request: NSFetchRequest<T>) {
		...
		// Return deleted object IDs and merge them into the live context so @FetchRequest
		// views update immediately (otherwise the rows linger until the app restarts).
