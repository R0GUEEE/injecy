...
class DownloadManager: NSObject, ObservableObject {
	...
	func startDownload(
		...
		LiveActivityManager.shared.start(
			id: id, name: url.lastPathComponent,
			phase: .downloading, status: .localized("Downloading")
		)
	...
	func startArchive(
		...
		LiveActivityManager.shared.start(
			...
			phase: .importing, status: .localized("Importing")
		)
	...
	func cancelDownload(_ download: Download) {
		...
		if let index = downloads.firstIndex(where: { $0.id == download.id }) {
			...
			LiveActivityManager.shared.end(id: download.id, phase: .failed, status: .localized("Cancelled"))
		}
	}
...
extension DownloadManager: URLSessionDownloadDelegate {
	...
	func handlePachageFile(url: URL, dl: Download) throws {
		LiveActivityManager.shared.update(id: dl.id, phase: .importing, status: .localized("Importing"), progress: 1.0)
		FR.handlePackageFile(url, download: dl) { err in
			if err != nil {
				let generator = UINotificationFeedbackGenerator()
				generator.notificationOccurred(.error)
				LiveActivityManager.shared.end(id: dl.id, phase: .failed, status: .localized("Failed"))
			} else {
				LiveActivityManager.shared.end(id: dl.id, phase: .done, status: .localized("Ready to install"))
			}
	...
	func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didWriteData bytesWritten: Int64, totalBytesWritten: Int64, totalBytesExpectedToWrite: Int64) {
		...
		DispatchQueue.main.async {
			...
			LiveActivityManager.shared.update(
				id: download.id, phase: .downloading,
				status: .localized("Downloading"),
