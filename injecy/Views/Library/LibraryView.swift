...
private struct ImportProgressRow: View {
	...
	var body: some View {
		VStack(alignment: .leading, spacing: 7) {
			HStack(spacing: 10) {
				Image(systemName: download.onlyArchiving ? "archivebox.fill" : "arrow.down.circle.fill")
					.foregroundStyle(Color.accentColor)
				Text(download.fileName)
					.font(.subheadline.weight(.medium))
					.lineLimit(1)
				Spacer(minLength: 8)
				Text("\(Int((download.overallProgress * 100).rounded()))%")
					.font(.caption.monospacedDigit())
					.foregroundStyle(.secondary)
			}
			ProgressView(value: min(max(download.overallProgress, 0), 1))
				.tint(.accentColor)
			Text(_phase)
				.font(.caption2)
				.foregroundStyle(.secondary)
	...
	private var _phase: String {
		if !download.onlyArchiving, download.progress < 1 {
			return .localized("Downloading")
		}
		return download.unpackageProgress >= 1
