...
enum FR {
	...
	static func signPackageFile(
		...
		let jobId = "sign_\(UUID().uuidString)"
		LiveActivityManager.shared.start(
			id: jobId, name: app.name ?? .localized("App"),
			phase: .signing, status: .localized("Signing")
		)
		Task.detached {
			...
			do {
				...
				try? await handler.clean()
				LiveActivityManager.shared.end(id: jobId, phase: .done, status: .localized("Signed"))
				await MainActor.run {
					completion(nil)
				}
			} catch {
				try? await handler.clean()
				LiveActivityManager.shared.end(id: jobId, phase: .failed, status: .localized("Failed"))
				await MainActor.run {
					completion(error)
				}
			}
