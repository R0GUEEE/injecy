...
final class ArchiveHandler: NSObject {
	...
	static func getCompressionLevel() -> Int {
		// Default to real compression when unset — otherwise integer(forKey:) returns 0
		// (NoCompression), which bloats archives ~3x (e.g. a 21 MB IPA → 67 MB).
		UserDefaults.standard.object(forKey: "Feather.compressionLevel") as? Int
			?? ZipCompression.DefaultCompression.rawValue
	}
}
